...
@php
    ...
    function inventarioPagUrl($pagina, $buscar, $estado)
    {
        ...
        return route('vendedor.inventario', $params);
    }
@endphp
...
<div class="max-w-7xl mx-auto font-sans-ventanet">
    ...
    <form method="GET" action="{{ route('vendedor.inventario') }}" class="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        @if ($buscar)
        <a href="{{ route('vendedor.inventario', ['estado' => $estado]) }}"
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px;">
        <a href="{{ route('vendedor.inventario', ['buscar' => $buscar]) }}"
           class="filtro-link {{ $estado === '' ? 'activo' : '' }}">
            Todos
        </a>
        <a href="{{ route('vendedor.inventario', ['buscar' => $buscar, 'estado' => 'disponible']) }}"
           class="filtro-link {{ $estado === 'disponible' ? 'activo' : '' }}">
            ✓ Disponible
        </a>
        <a href="{{ route('vendedor.inventario', ['buscar' => $buscar, 'estado' => 'bajo']) }}"
           class="filtro-link {{ $estado === 'bajo' ? 'activo-bajo' : '' }}">
            ⚠ Stock bajo
        </a>
        <a href="{{ route('vendedor.inventario', ['buscar' => $buscar, 'estado' => 'agotado']) }}"
                    ...
                    @forelse ($inventario as $row)
                        ...
                        <tr style="border-bottom:1px solid #E5E7EB; background:{{ $bgFila }}; transition:background .15s;"
                            ...
                            <td class="px-5 py-3.5 text-center">
                                ...
                                @endphp
                                <button type="button"
                                    onclick='abrirModalActualizar(@json($infoStock))'
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="padding:48px;text-align:center;">
                                ...
                                @if ($buscar || $estado)
                                <a href="{{ route('vendedor.inventario') }}" style="color:#00875F;font-size:.85rem;font-weight:600;margin-top:8px;display:inline-block;">
        ...
        @if ($paginas > 1)
            <div style="padding:14px 20px;border-top:1px solid #E5E7EB;display:flex;align-items:center;justify-content:center;flex-wrap:wrap;">
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <a href="{{ inventarioPagUrl($pagina - 1, $buscar, $estado) }}" class="pag-btn {{ $pagina <= 1 ? 'deshabilitado' : '' }}">
                        ← Anterior
                    </a>
                    @for ($pg = 1; $pg <= $paginas; $pg++)
                        <a href="{{ inventarioPagUrl($pg, $buscar, $estado) }}" class="pag-btn {{ $pg === $pagina ? 'activa' : '' }}">
                            {{ $pg }}
                        </a>
                    @endfor
                    <a href="{{ inventarioPagUrl($pagina + 1, $buscar, $estado) }}" class="pag-btn {{ $pagina >= $paginas ? 'deshabilitado' : '' }}">
                        Siguiente →
                    </a>
                </div>
            </div>
...
<div id="modalActualizar" class="fixed inset-0 hidden z-50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl shadow-2xl modal-anim overflow-hidden" style="width:100%;max-width:420px;">
        ...
        <!-- Formulario -->
        <form id="formActualizarStock" action="" method="POST"
...
<script>
    // URL base con placeholder de ID, generada por Laravel
    var urlActualizarBase = "{{ route('vendedor.inventario.actualizar', ['id' => 'ID_PLACEHOLDER']) }}";

    function abrirModalActualizar(datos) {
        document.getElementById('formActualizarStock').action = urlActualizarBase.replace('ID_PLACEHOLDER', datos.id_producto);
