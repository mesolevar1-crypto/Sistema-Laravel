                    ...
                    @forelse ($ventas as $v)
                        ...
                        >
                            <td class="px-5 py-3.5 text-sm font-medium" style="color:#171717;">
                                {{ !empty($v['fecha']) ? \Carbon\Carbon::parse($v['fecha'])->format('d/m/Y') : '---' }}
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div style="display:flex;align-items:center;justify-content:center;gap:8px;">
                                    ...
                                    @if ($activa)
                                        <button
                                            type="button"
                                            onclick="abrirModalEliminar({{ (int) $v['id_venta'] }})"
                                            title="Anular venta"
                                            class="btn-accion-cuadro"
                                    ...
                                    @else
                                        <button
                                            type="button"
                                            onclick="abrirModalReactivar({{ (int) $v['id_venta'] }})"
...
<div id="modalCrear" class="fixed inset-0 hidden z-50 flex items-center justify-center p-4" style="background:rgba(0,0,0,.50);backdrop-filter:blur(4px);">
    <div class="bg-white rounded-2xl w-full shadow-2xl modal-anim overflow-hidden" style="max-width:1120px;max-height:92vh;display:flex;flex-direction:column;">
        ...
        <form
            id="formVenta"
            action="{{ route('vendedor.ventas.store') }}"
            method="POST"
...
<div id="modalFactura" class="fixed inset-0 hidden z-50 flex items-center justify-center p-4" style="background:rgba(0,0,0,.60);backdrop-filter:blur(4px);">
    <div class="bg-white rounded-2xl w-full shadow-2xl modal-anim overflow-hidden" style="max-width:620px;max-height:94vh;display:flex;flex-direction:column;">
        ...
        <div style="background:#fff;border-top:1px solid #E5E7EB;padding:13px 22px;display:flex;justify-content:flex-end;gap:9px;flex-shrink:0;">
            <button type="button" id="btnExportarFacturaPdf" onclick="exportarFacturaPdf()" class="btn-exito" style="padding:8px 18px;">
...
<!-- Formularios ocultos para anular/reactivar (Laravel requiere POST, no GET) -->
<form id="formAnular" action="" method="POST" style="display:none">
    @csrf
</form>

<form id="formReactivar" action="" method="POST" style="display:none">
    @csrf
</form>
...
var urlDetalleBase     = "{{ route('vendedor.ventas.detalle', ['id' => 'ID_PLACEHOLDER']) }}";
var urlAnularBase      = "{{ route('vendedor.ventas.anular', ['id' => 'ID_PLACEHOLDER']) }}";
var urlReactivarBase   = "{{ route('vendedor.ventas.reactivar', ['id' => 'ID_PLACEHOLDER']) }}";
var urlFacturaPdfBase  = "{{ route('vendedor.ventas.factura.pdf', ['id' => 'ID_PLACEHOLDER']) }}";

var filaCount = 0;
var idVentaSeleccionada = null;
...
function confirmarAnular() {
    if (!idVentaSeleccionada) return;
    var form = document.getElementById('formAnular');
    form.action = urlAnularBase.replace('ID_PLACEHOLDER', idVentaSeleccionada);
    form.submit();
}
...
function confirmarReactivar() {
    ...
    form.action = urlReactivarBase.replace('ID_PLACEHOLDER', idVentaSeleccionada);
    form.submit();
}
...
// Botón "Exportar a PDF" del modal de factura: descarga el mismo
// PDF (dompdf) que ya genera facturaPdf() en el controlador, usando
// el id de la venta que está actualmente abierta en el modal.
function exportarFacturaPdf() {
    if (!idFacturaActual) return;
    window.open(urlFacturaPdfBase.replace('ID_PLACEHOLDER', idFacturaActual), '_blank');
}
...
function verDetalle(id) {
    ...
    fetch(urlDetalleBase.replace('ID_PLACEHOLDER', id))
