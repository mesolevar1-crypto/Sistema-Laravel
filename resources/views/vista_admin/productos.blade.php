...
@php
    ...
    function colorPorCategoria($idCategoria, $paleta) {
...
.ventanet-input, .ventanet-select {
    background: #fff; border: 1px solid #dfe7df; border-radius: 10px; outline: none;
}
.ventanet-input:focus, .ventanet-select:focus {
    border-color: #61d0a7; box-shadow: 0 0 0 3px rgba(97,208,167,.15);
}
...
<div class="font-sans-ventanet text-[#1C2E1A] max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <button type="button" onclick="openModal('modalCategorias')"
                class="ventanet-btn-secondary px-5 py-2.5 flex items-center gap-2 justify-center">
                <i class="fas fa-tags"></i> Agregar Categorías
            </button>
    ...
    <div id="gridProductos" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        @forelse ($productos as $p)
            @php
                ...
                $estadoActivo = (bool) ($p['estado'] ?? false);
                $colorBorde = $estadoActivo ? $color['accent'] : '#ef4444';
            @endphp

            <div class="product-card producto-item {{ $estadoActivo ? '' : 'inactivo' }}"
                 data-nombre="{{ Str::lower($p['nombre']) }}"
                 data-categoria="{{ Str::lower($p['categoria'] ?? '') }}"
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between gap-2 mt-4">
                        ...
                        @if (!empty($p['categoria']))
                            <span class="shrink-0 px-2.5 py-1 rounded-full text-[.68rem] font-bold border"
                    ...
                    <div class="border-t border-[#C9E4C5] mt-4 pt-4">
                        <div class="product-actions">
                            ...
                            <button type="button" class="product-action action-edit"
                                onclick='openEditModal(@json($p))' title="Editar producto">
                                <i class="fas fa-pen"></i>
                            </button>

                            <button type="button" class="product-action action-status"
                                onclick="cambiarEstado({{ (int) $p['id_producto'] }}, {{ $estadoActivo ? 'true' : 'false' }})"
                                title="{{ $estadoActivo ? 'Desactivar producto' : 'Activar producto' }}">
                                <i class="fas fa-power-off"></i>
                            </button>

                            <button type="button" class="product-action action-delete"
                                onclick='openDeleteModal({{ (int) $p["id_producto"] }}, @json($p["nombre"]))'
    ...
    <!-- PAGINACION (JS, sobre el listado ya cargado) -->
...
<div id="modalCrear" class="fixed inset-0 bg-[#1C2E1A]/50 backdrop-blur-sm hidden z-[9999] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] shadow-2xl modal-container max-w-lg">
        ...
        <form action="{{ route('admin.productos.store') }}" method="POST" enctype="multipart/form-data" class="p-7 space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-[#C9E4C5] rounded-xl p-4 text-center cursor-pointer"
...
<div id="modalEditar" class="fixed inset-0 bg-[#1C2E1A]/50 backdrop-blur-sm hidden z-[9999] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] shadow-2xl modal-container max-w-lg">
        <div class="px-7 py-5 border-b flex justify-between items-center bg-[#F8F8F8]">
            ...
            <button type="button" onclick="closeModal('modalEditar')"><i class="fas fa-times text-xl"></i></button>
        </div>

        <form id="formEditarProducto" action="" method="POST" enctype="multipart/form-data" class="p-7 space-y-5">
            @csrf
...
<div id="modalEliminar" class="fixed inset-0 bg-[#1C2E1A]/60 backdrop-blur-sm hidden z-[10000] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] w-full max-w-md shadow-2xl">
        ...
        <div class="px-6 py-5 bg-[#F8F8F8] border-t flex justify-center gap-3">
            ...
            <button type="button" onclick="confirmarEliminarProducto()" class="ventanet-btn-danger px-6 py-2.5 flex items-center">
                Sí, eliminar
            </button>
        </div>
    </div>
...
<div id="modalCategorias" class="fixed inset-0 bg-[#1C2E1A]/50 backdrop-blur-sm hidden z-[9999] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] shadow-2xl modal-container max-w-lg">
        ...
        <div class="p-7 space-y-4">
            <button type="button" onclick="closeModal('modalCategorias'); openModal('modalCrearCategoria');"
                ...
                @forelse ($categorias as $cat)
                    ...
                    <div class="categoria-item">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <button type="button" class="product-action action-edit" style="height:36px; width:36px;"
                                title="Editar categoría" onclick='openEditCategoriaModal(@json($cat))'>
                                <i class="fas fa-pen"></i>
                            </button>
                            <button type="button" class="product-action action-delete" style="height:36px; width:36px;"
                                title="Eliminar categoría" onclick='openDeleteCategoriaModal({{ (int) $cat["id_categoria"] }}, @json($cat["tipo"]))'>
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
...
<div id="modalCrearCategoria" class="fixed inset-0 bg-[#1C2E1A]/50 backdrop-blur-sm hidden z-[10000] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] shadow-2xl modal-container max-w-sm">
        ...
        <form action="{{ route('admin.categorias.store') }}" method="POST" class="p-7 space-y-5">
            @csrf
            <div>
...
<div id="modalEditarCategoria" class="fixed inset-0 bg-[#1C2E1A]/50 backdrop-blur-sm hidden z-[10000] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] shadow-2xl modal-container max-w-sm">
        ...
        <form id="formEditarCategoria" action="" method="POST" class="p-7 space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-bold mb-1.5">Nombre de la Categoría *</label>
...
<div id="modalEliminarCategoria" class="fixed inset-0 bg-[#1C2E1A]/60 backdrop-blur-sm hidden z-[10001] modal-overlay flex items-center justify-center">
    <div class="bg-white rounded-[20px] w-full max-w-md shadow-2xl">
        ...
        <div class="px-6 py-5 bg-[#F8F8F8] border-t flex justify-center gap-3">
            ...
            <button type="button" onclick="confirmarEliminarCategoria()" class="ventanet-btn-danger px-6 py-2.5 flex items-center">
                Sí, eliminar
            </button>
        </div>
    </div>
...
var baseProductos = "{{ url('/admin/productos') }}";
var baseCategorias = "{{ url('/admin/categorias') }}";
...
function openEditModal(p) {
    document.getElementById('formEditarProducto').action = baseProductos + '/' + p.id_producto + '/editar';
...
function cambiarEstado(id, estabaActivo) {

    const texto = estabaActivo
        ? '¿Quieres desactivar este producto?'
        : '¿Quieres activar este producto?';

    Swal.fire({
        icon: 'question',
        title: estabaActivo ? 'Desactivar producto' : 'Activar producto',
        text: texto,
        ...
        form.action = baseProductos + '/' + id + '/toggle';
        form.submit();
    });
}
...
function confirmarEliminarProducto() {
    ...
    form.action = baseProductos + '/' + productoEliminar;
    form.submit();
}
...
function openEditCategoriaModal(cat) {
    document.getElementById('formEditarCategoria').action = baseCategorias + '/' + cat.id_categoria + '/editar';
...
function confirmarEliminarCategoria() {
    ...
    form.action = baseCategorias + '/' + categoriaEliminar;
    form.submit();
}
